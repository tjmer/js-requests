...
function colorChange (){
    sayHelloButton.classList.add("btn")
}
sayHelloButton.addEventListener("mouseover", colorChange)
...
function changeBack(){
    sayHelloButton.classList.remove(`btn`)
}
...
const repeatMyParam = () => {
   ...
   .then(res=> {
       console.log(res.data)
       let again = document.getElementById('repeat-text');
        again.style.display = 'block';
        again.textContent = res.data;
   })
}
...
function request (){
    axios.get(`http://localhost:3000/query-test/?age=20`)
    .then((res) =>{
        console.log(res.data)
    }).catch(error=> {
        console.log(error)
...
/*
    ...
    1: Send no queries on the URL -- what happened? 
        It came back with "You sent an empty query"

    2: Send more than 1 query on the URL -- what happened? 
        It came back with "You sent more then 1 query"
*/
...
//Worked with clint, jack, michi and dylan.
